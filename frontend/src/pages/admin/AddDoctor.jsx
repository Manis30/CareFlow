...
const AddDoctor = () => {
  ...
  const fetchDepartments = async () => {
    try {
      ...
      if (depts.length > 0 && selectedDeptIds.length === 0) {
        setSelectedDeptIds([depts[0]._id]);
      }
    } catch (err) {
    ...
    switch (name) {
      case 'name':
        if (!valStr) {
          ...
          err = 'Clinician name must contain letters and spaces only.';
        }
        break;
  ...
  const validateAll = () => {
    ...
    if (profileImage) {
      if (profileImage.size > 5 * 1024 * 1024) {
        newErrors.profileImage = 'Profile photo must be under 5MB.';
      } else if (!profileImage.type.startsWith('image/')) {
        newErrors.profileImage = 'Only image files (PNG, JPG, WEBP) are allowed.';
      }
    }
  ...
  const handleImageChange = (e) => {
    ...
    if (!file.type.startsWith('image/')) {
      setErrors((prev) => ({ ...prev, profileImage: 'Only image files (PNG, JPG, WEBP) are allowed.' }));
      return;
    }
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      await createDoctorApi(bodyData);
      showSuccessToast('Clinician registered successfully!');
      navigate('/admin/doctors');
    } catch (err) {
      const errMsg = err.response?.data?.message || err.message || 'Failed to register clinician';
      setError(errMsg);
  ...
  return (
    <ContentContainer maxWidth="5xl">
      <PageHeader
        ...
        subtitle="Add a licensed medical specialist to your healthcare clinic organization."
        backButton={{
          ...
          <div className="flex items-center gap-4 w-full sm:w-auto">
            ...
            <div className="min-w-0 flex-1 space-y-1">
              <h3 className="text-lg font-bold text-slate-900 truncate tracking-tight">
                {formData.name.trim() ? formatDoctorName(cleanDoctorNameInput(formData.name)) : 'New Clinician Profile'}
              </h3>
              <p className="text-xs font-semibold text-blue-600 truncate">
                {formData.specialization.trim() ? formData.specialization : 'Specialization pending'}
              </p>
              {touched.profileImage && errors.profileImage && (
                <p className="text-xs text-rose-600 font-medium">{errors.profileImage}</p>
              )}
            </div>
          </div>
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-5">
            ...
            <div className="space-y-5">
              ...
              {departments.length > 0 ? (
                ...
                <div className="space-y-1.5">
                  ...
                  <div className="p-3 bg-blue-50/70 border border-blue-200/60 rounded-xl text-xs text-blue-800 font-medium">
                    No departments created yet. Clinician will be assigned to General Practice.
                  </div>
                </div>
              )}
              ...
              <div className="space-y-1.5">
                <label className="block text-xs font-semibold text-slate-700">
                  Account Initial Password <span className="text-rose-500">*</span>
                </label>
                <div className="relative">
                  ...
                  <input
                    ...
                    } rounded-xl text-xs sm:text-sm text-slate-900 font-medium transition-all outline-none`}
                  />
                  <button
                    type="button"
